...
def main():
    ...
    run_remote("cd casper-voice-web && rm -rf .next && npm install && npx prisma generate && npx prisma db push --accept-data-loss && node scripts/backfill_null_tenants.js && npx tsx scripts/clean_long_tenant_names.ts && npx tsx scripts/migrate_tenant_admin_chat_id.ts && npm run build")
