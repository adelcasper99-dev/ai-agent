...
|-------|------------------------------------------|
| `review:ce-prisma-drift-detector` | Cross-references schema.prisma against included migrations |
...
For CE conditional agents, check if the diff includes files matching `prisma/schema.prisma`, `prisma/migrations/**/*`, or data backfill scripts.
...
- ce-prisma-drift-detector -- migration files present
```
...
CE always-on agents (ce-agent-native-reviewer, ce-learnings-researcher) and CE conditional agents (ce-prisma-drift-detector, ce-deployment-verification-agent) also use the mid-tier model since they perform scoped, focused work.
...
```bash
RUN_ID=$(date +%Y%m%d-%H%M%S)-$(node -e "console.log(require('crypto').randomBytes(4).toString('hex'))")
...
**CE conditional agents** (ce-prisma-drift-detector, ce-deployment-verification-agent) are also dispatched as standard Agent calls when applicable. Pass the same review context bundle plus the applicability reason (for example, which migration files triggered the agent). For ce-prisma-drift-detector specifically, pass the resolved review base branch explicitly so it never assumes `main`. Their output is unstructured and must be preserved for Stage 6 synthesis just like the CE always-on agents.
...
11. **Preserve CE agent artifacts.** Keep the learnings, agent-native, prisma-drift, and deployment-verification outputs alongside the merged finding set. Do not drop unstructured agent output just because it does not match the persona JSON schema.
...
9. **Prisma Schema Drift Check.** If ce-prisma-drift-detector ran, summarize whether drift was found. If drift exists, list the unrelated schema changes and the required cleanup command. If clean, say so briefly.
