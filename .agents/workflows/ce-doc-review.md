...
**If no document is specified (headless mode):** Automatically search for the most recent document in `docs/brainstorms/` or `docs/plans/` and proceed. If no document exists in the workspace, output "Review failed: no document found in docs/brainstorms/ or docs/plans/".
...
### ⚡ Automatic File Updating & Chat Suppression Protocol
1. **Auto-Patch Target File on Disk**:
   - For all auto-fixable findings and approved mitigations, **automatically update the target document file on disk** (`docs/brainstorms/*.md` or `docs/plans/*.md`).
   - Use **Section-Anchored Delta Edits** targeting specific `## Section` headings only to prevent overwriting un-edited user content.
2. **Suppress Full Document in Chat Output**:
   - Do **NOT** dump the full modified document body into the chat response stream.
   - Replace chat document dumps with a concise **5-Line Patch Summary** formatted as:
     ```markdown
     📝 **Target File Updated**: [doc-basename.md](file:///path/to/doc.md)

     **Applied Changes:**
     - ⚠️ [ADDED] Section X: Description of addition
     - ⚠️ [MODIFIED] Section Y: Description of modification
     - ⚠️ [REMOVED] Section Z: Description of removal
     ```
3. **Headless vs Interactive Handoff**:
   - In `mode:headless`, return immediately after updating the target file.
   - In interactive mode, output the 5-Line Patch Summary and present next action options.
