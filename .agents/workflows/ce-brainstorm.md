...
Write or update a requirements document directly to `docs/brainstorms/YYYY-MM-DD-<topic>-requirements.md`. Read `references/requirements-capture.md` for the document template, formatting rules, visual aid guidance, and completeness checks.

### ⚡ Direct File Writing & Chat Suppression Protocol
- **Direct File Persistence**: Always write the full requirements document directly to disk under `docs/brainstorms/`.
- **Suppress Chat Token Dump**: Do **NOT** dump the full requirements document body into the chat stream. In chat, output only a compact 3-bullet overview and a clickable file link (e.g. `[YYYY-MM-DD-topic-requirements.md](file:///path/to/docs/brainstorms/file.md)`).
...
Present next-step options and execute the user's selection. Read `references/handoff.md` for the option logic, dispatch instructions, and closing summary format. Output file link prominently.
