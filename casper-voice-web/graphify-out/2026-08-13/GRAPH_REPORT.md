...
5. `isInternalAuthValid()` - 14 edges
6. `sendTelegramAlert()` - 14 edges
7. `executeTool()` - 13 edges
8. `approveTenantRequest()` - 10 edges
9. `fireAndForgetTelegramAlert()` - 8 edges
10. `handleFallbackSaleCallback()` - 8 edges
...
Cohesion: 0.10
Nodes (25): addProductTool, ARABIC_NUMBER_WORDS, bookAppointmentTool, executedKeys, FINANCIAL_TOOLS, findCustomerFuzzy(), getAppointmentsListTool, getCustomerBalanceTool (+17 more)
...
Cohesion: 0.14
Nodes (12): executeTool(), logRejectedToolCall(), { executeTool }, prisma, { PrismaClient }, runTests(), { executeTool }, prisma (+4 more)
