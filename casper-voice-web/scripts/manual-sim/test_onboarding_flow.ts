...
async function runOnboardingTest() {
  ...
  // 1. Initial Cleanup
  await prisma.tenant.deleteMany({ where: { telegramChatId: TEST_CHAT_ID } });
  ...
  console.log(`✅ State transitioned to: ${tenant2.state}`);

  // 4. Stage 3: Provide Merchant Name
  console.log("\n▶ STAGE 3: Merchant inputs name ('أستاذ محمود العوضي')");
  const cleanMerchantName = "محمود العوضي"; // Stripped 'أستاذ'
  const tenant3 = await prisma.tenant.update({
    where: { id: tenant1.id },
    data: {
      merchantName: cleanMerchantName,
      state: "onboarding_business_name",
    },
  });
  console.log(`✅ Merchant Name saved: '${tenant3.merchantName}' | State: ${tenant3.state}`);

  // 5. Stage 4: Provide Business Name
  console.log("\n▶ STAGE 4: Merchant inputs business name ('شركة الأمل لمواد البناء')");
  ...
  console.log(`✅ Tenant Approval Completed | Final State: ${approvedTenant.state}`);
  ...
  if (finalTenant?.state === "active" && finalTenant.merchantName === cleanMerchantName) {
